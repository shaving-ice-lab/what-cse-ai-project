...
const filterConfig = {
  examType: {
    label: "考试类型",
    options: ["国考", "省考", "事业单位", "选调生", "军队文职", "三支一扶"],
  },
  province: {
    label: "省份",
    options: [
  ...
  positionType: {
    label: "职位类别",
    options: ["综合管理类", "行政执法类", "专业技术类", "司法类", "公安类"],
  },
  ...
  identity: {
    label: "身份要求",
    options: ["不限", "应届生", "在职人员", "服务基层项目人员", "退役军人"],
  },
  ageRange: {
    label: "年龄限制",
    options: ["不限", "18-25岁", "18-30岁", "18-35岁", "18-40岁", "40岁以上"],
  },
};
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <input
        ...
        className="w-14 px-1 py-0.5 text-xs border border-stone-200 rounded-lg text-center focus:border-amber-500 outline-none"
        min={min}
        max={max}
      />
...
export default function PositionsPage() {
  ...
  const updateRange = (key: string, val: [number, number]) =>
  ...
  const filteredData = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      data = data.filter(
        (p) => p.name.includes(q) || p.dept.includes(q) || p.code.includes(q) || p.major.includes(q)
      );
    }
    ...
    if (filters.positionType.length)
      data = data.filter((p) => filters.positionType.includes(p.positionType));
    ...
    if (filters.majorType.length)
      data = data.filter((p) => filters.majorType.includes(p.majorType));
    if (filters.political.length)
      data = data.filter((p) => filters.political.includes(p.political));
  ...
  const stats = useMemo(
    () => ({
      ...
      avgRatio: filteredData.length
        ? Math.round(filteredData.reduce((s, p) => s + p.ratio, 0) / filteredData.length)
        : 0,
      ...
      <div className="bg-white/80 backdrop-blur-lg border-b border-stone-200/50 sticky top-0 z-30 shadow-warm-sm">
        <div className="max-w-[1800px] mx-auto">
          ...
          <div className="flex items-center gap-2 px-3 py-2 flex-wrap">
            ...
            <MultiSelect
              label="专业大类"
              options={filterConfig.majorType.options}
              selected={filters.majorType}
              onChange={(v) => updateFilter("majorType", v)}
            />
            <MultiSelect
              label="政治面貌"
              options={filterConfig.political.options}
              selected={filters.political}
              onChange={(v) => updateFilter("political", v)}
            />
            ...
            {/* 工具按钮 */}
            <button className="p-1.5 text-stone-500 hover:bg-stone-100 rounded-lg">
          ...
          {showAdvanced && (
            <div className="border-t border-stone-100 bg-stone-50/50">
              ...
              <div className="flex items-center gap-2 px-3 py-2 flex-wrap">
                ...
                <MultiSelect
                  label="部门类型"
                  options={filterConfig.deptType.options}
                  selected={filters.deptType}
                  onChange={(v) => updateFilter("deptType", v)}
                />
                <MultiSelect
                  label="职位类别"
                  options={filterConfig.positionType.options}
                  selected={filters.positionType}
                  onChange={(v) => updateFilter("positionType", v)}
                />
                <MultiSelect
                  label="学位要求"
                  options={filterConfig.degree.options}
                  selected={filters.degree}
                  onChange={(v) => updateFilter("degree", v)}
                />
                <MultiSelect
                  label="基层经历"
                  options={filterConfig.workExp.options}
                  selected={filters.workExp}
                  onChange={(v) => updateFilter("workExp", v)}
                />
                <MultiSelect
                  label="身份要求"
                  options={filterConfig.identity.options}
                  selected={filters.identity}
                  onChange={(v) => updateFilter("identity", v)}
                />
                <MultiSelect
                  label="性别要求"
              ...
              <div className="flex items-center gap-4 px-3 py-2 border-t border-stone-100 flex-wrap">
                ...
                <RangeFilter
                  ...
                  min={1}
                  max={100}
                  value={ranges.recruit}
                  onChange={(v) => updateRange("recruit", v)}
                />
                <RangeFilter
                  label="竞争比"
                  min={0}
                  max={1000}
                  value={ranges.ratio}
                  onChange={(v) => updateRange("ratio", v)}
                />
                <RangeFilter
                  label="匹配度"
                  min={0}
                  max={100}
                  value={ranges.match}
                  onChange={(v) => updateRange("match", v)}
                />
                <RangeFilter
                  label="分数线"
                  min={80}
                  max={180}
                  value={ranges.score}
                  onChange={(v) => updateRange("score", v)}
                />
              </div>
          ...
          {compareList.length > 0 && (
            <div className="flex items-center gap-2 px-3 py-1.5 border-t border-stone-100 bg-emerald-50">
              ...
              <button className="px-2 py-0.5 bg-emerald-600 text-white text-xs rounded-lg hover:bg-emerald-700">
                开始对比
              </button>
              <button
        ...
        <div className="bg-white border border-stone-200/50 rounded-2xl overflow-hidden shadow-card">
            ...
            <table className="w-full text-sm border-collapse">
              <thead className="bg-stone-50">
                <tr>
                  ...
                  <th className="px-1.5 py-2.5 text-left font-semibold text-stone-600 border-b border-stone-200">
                    代码
                  </th>
                  <th className="px-1.5 py-2.5 text-left font-semibold text-stone-600 border-b border-stone-200 min-w-[140px]">
                    职位名称
                  </th>
                  <th className="px-1.5 py-2.5 text-left font-semibold text-stone-600 border-b border-stone-200 min-w-[180px]">
                    招录单位
                  </th>
                  <th className="px-1.5 py-2.5 text-center font-semibold text-stone-600 border-b border-stone-200">
                    类型
                  </th>
                  <th className="px-1.5 py-2.5 text-center font-semibold text-stone-600 border-b border-stone-200">
                    层级
                  ...
                  <th
                    className="px-1.5 py-2.5 text-center font-semibold text-stone-600 border-b border-stone-200 cursor-pointer hover:bg-stone-100 rounded-lg"
                    onClick={() => handleSort("recruit")}
                  >
                    招录
                    {sortKey === "recruit" &&
                      (sortAsc ? (
                  ...
                  <th className="px-1.5 py-2.5 text-center font-semibold text-stone-600 border-b border-stone-200 w-16">
                    操作
                  </th>
                </tr>
              </thead>
              <tbody>
                {filteredData.map((p, idx) => (
                  ...
                  >
                    ...
                    <td className="px-1.5 py-1.5">
                      <div className="truncate text-stone-700" title={p.dept}>
                        {p.dept}
                      </div>
                    </td>
                    ...
                    <td className="px-1.5 py-1.5 text-stone-700 text-xs">
                      {p.province} {p.city}
                    </td>
                    <td className="px-1.5 py-1.5 text-center text-stone-600">{p.edu}</td>
                    <td className="px-1.5 py-1.5">
                      <span
                        className={`text-xs truncate ${p.major === "不限" ? "text-emerald-600 font-medium" : "text-stone-600"}`}
                        title={p.major}
                      >
                    ...
                    <td className="px-1.5 py-1.5 text-center">
                      <span
                        className={`font-mono font-bold ${p.recruit >= 5 ? "text-emerald-600" : p.recruit >= 3 ? "text-green-500" : "text-stone-700"}`}
                      >
                        {p.recruit}
                      </span>
                    </td>
                    <td className="px-1.5 py-1.5 text-center">
                      <span
                        className={`font-mono font-bold px-1.5 py-0.5 rounded ${ratioColor(p.ratio)} ${ratioBg(p.ratio)}`}
                      >
                        {p.ratio}:1
                      </span>
                    </td>
                    <td className="px-1.5 py-1.5 text-center">
                      <div className="flex items-center justify-center gap-1">
                        <span className={`font-mono font-bold ${matchColor(p.match)}`}>
                          {p.match}%
                        </span>
                        <div className="w-8 h-1.5 bg-stone-200 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full ${matchBg(p.match)}`}
                            style={{ width: `${p.match}%` }}
                          />
                        </div>
                      </div>
                    ...
                    <td className="px-1.5 py-1.5 text-center">
                      <span
                        className={`font-mono text-xs flex items-center justify-center gap-0.5 ${p.trend > 0 ? "text-red-500" : p.trend < 0 ? "text-emerald-500" : "text-stone-400"}`}
                      >
                        {p.trend > 0 ? (
                    ...
                    <td className="px-1.5 py-1.5 text-center font-mono text-amber-600 text-xs">
                      {p.score}
                    </td>
                    <td className="px-1.5 py-1.5">
                      <div className="flex flex-wrap gap-0.5 justify-center">
                        ...
                        {p.identity !== "不限" && (
                          <span className="px-1 py-0.5 bg-blue-50 text-blue-600 rounded text-xs">
                            {p.identity}
                          </span>
                        )}
                      </div>
                    ...
                    <td className="px-1.5 py-1.5">
                      <div className="flex items-center justify-center gap-0.5">
                        <button
                          ...
                          className={`p-1 rounded-lg transition-colors ${favorites.includes(p.id) ? "text-amber-500" : "text-stone-400 hover:text-amber-500"}`}
                        >
                          <Star
                            className={`w-4 h-4 ${favorites.includes(p.id) ? "fill-current" : ""}`}
                          />
                        </button>
                        <Link
                          href={`/positions/${p.id}`}
          ...
          <div className="bg-stone-50 border-t border-stone-200 px-3 py-2.5 flex items-center justify-between text-sm">
            <div className="flex items-center gap-4 text-stone-600">
              <span>
                共 <b className="text-stone-800">{filteredData.length}</b> 条
              </span>
              <span>
                招录 <b className="text-emerald-600">{stats.recruit}</b> 人
              </span>
              <span>
                均竞争 <b className="text-amber-600">{stats.avgRatio}:1</b>
              </span>
            </div>
