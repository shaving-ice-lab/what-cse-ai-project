...
const filterConfig = {
  type: {
    label: "公告类型",
    options: ["招考公告", "报名通知", "时间公告", "考试指南", "政策解读", "成绩查询", "面试公告"],
  },
  examType: {
    label: "考试类型",
  ...
  source: {
    label: "发布机构",
    options: ["国家公务员局", "人社部", "省人社厅", "市人社局", "组织部", "其他"],
  },
...
const announcements = [
  ...
  },
  {
    id: 13,
    code: "GK2024-004",
    title: "中央机关及其直属机构2024年度考试录用公务员政策解读",
    type: "政策解读",
    examType: "国考",
    source: "国家公务员局",
    region: "全国",
...
export default function AnnouncementListPage() {
  ...
  const filteredData = useMemo(() => {
    ...
    if (filters.source.length)
      data = data.filter((a) => filters.source.some((s) => a.source.includes(s)));
      ...
      <div className="bg-white/80 backdrop-blur-lg border-b border-stone-200/50 sticky top-0 z-30 shadow-warm-sm">
        <div className="max-w-[1800px] mx-auto">
          ...
          <div className="flex items-center gap-2 px-3 py-2 flex-wrap">
            ...
            <MultiSelect
              label="地区"
              options={filterConfig.region.options}
          ...
          {showAdvanced && (
            <div className="border-t border-stone-100 bg-stone-50/50">
              ...
              <div className="flex items-center gap-2 px-3 py-2 flex-wrap">
                ...
                <MultiSelect
                  label="发布机构"
                  options={filterConfig.source.options}
        ...
        <div className="bg-white border border-stone-200/50 rounded-2xl overflow-hidden shadow-card">
            ...
            <table className="w-full text-sm border-collapse">
              <thead className="bg-stone-50">
                <tr>
                  ...
                  <th className="px-2 py-2.5 text-left font-semibold text-stone-600 border-b border-stone-200 min-w-[280px]">
                    公告标题
                  </th>
                  ...
                  <th className="px-2 py-2.5 text-left font-semibold text-stone-600 border-b border-stone-200 w-32">
                    来源
                  </th>
                  ...
                  <th
                    ...
                    onClick={() => handleSort("recruitCount")}
                  >
                    <div className="flex items-center justify-center gap-1">
                      招录
                      {sortKey === "recruitCount" &&
                        (sortAsc ? (
                          <ArrowUp className="w-3 h-3" />
                        ) : (
                          <ArrowDown className="w-3 h-3" />
                ...
                {filteredData.map((a, idx) => (
                  ...
                  >
                    ...
                    <td className="px-2 py-2 text-center">
                      <span
                        className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${typeColor(a.type)}`}
                      >
                        {a.type}
                      </span>
                    </td>
                    <td className="px-2 py-2 text-center">
                      <span
                        className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${examTypeColor(a.examType)}`}
                      >
                        {a.examType}
                      </span>
                    </td>
                    <td className="px-2 py-2">
                      <div className="flex items-center gap-1 text-stone-600 text-xs">
                        ...
                        <span className="truncate" title={a.source}>
                          {a.source.length > 8 ? a.source.slice(0, 8) + "…" : a.source}
                        </span>
                      </div>
                    </td>
                    <td className="px-2 py-2 text-center">
                    ...
                    <td className="px-2 py-2 text-center text-xs">
                      {a.deadline ? (
                        <span
                          className={`${new Date(a.deadline) < new Date() ? "text-stone-400" : "text-red-600 font-medium"}`}
                        >
                          {a.deadline.slice(5)}
                        </span>
                      ) : (
                    ...
                    <td className="px-2 py-2 text-center">
                      <span
                        className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${statusColor(a.status)}`}
                      >
                        {a.status}
                      </span>
                    </td>
                    <td className="px-2 py-2">
                      <div className="flex items-center justify-center gap-1">
                        <button
                          ...
                          className={`p-1 rounded-lg transition-colors ${favorites.includes(a.id) ? "text-amber-500" : "text-stone-400 hover:text-amber-500"}`}
                        >
                          <Star
                            className={`w-4 h-4 ${favorites.includes(a.id) ? "fill-current" : ""}`}
                          />
                        </button>
                        <Link
                          href={`/announcements/${a.id}`}
          ...
          <div className="bg-stone-50 border-t border-stone-200 px-3 py-2.5 flex items-center justify-between text-sm">
            <div className="flex items-center gap-4 text-stone-600">
              <span>
                共 <b className="text-stone-800">{stats.total}</b> 条
              </span>
              <span>
                招录 <b className="text-emerald-600">{formatNumber(stats.totalRecruit)}</b> 人
              </span>
              <span>
                总浏览 <b className="text-amber-600">{formatNumber(stats.totalViews)}</b>
              </span>
              <span>
                进行中 <b className="text-amber-600">{stats.recruiting}</b>
              </span>
            </div>
