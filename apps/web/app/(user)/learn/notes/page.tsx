...
const noteTypeColors: Record<
  NoteType,
  { bg: string; text: string; border: string; icon: LucideIcon; gradient: string }
> = {
  course: {
    bg: "bg-blue-50",
    text: "text-blue-600",
...
function StatCard({
  ...
  return (
    ...
    >
      {/* 背景装饰 */}
      <div
        className={`absolute -top-6 -right-6 w-16 h-16 rounded-full bg-gradient-to-br ${gradient || "from-amber-400/10 to-orange-400/10"} blur-xl group-hover:scale-125 transition-transform duration-500`}
      ></div>

      <div className="relative flex items-center gap-2.5">
        <div
          className={`w-9 h-9 rounded-lg ${color} flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform duration-300`}
        >
          <Icon className="w-4 h-4" />
        </div>
        <div>
          <div className="text-xl font-bold text-stone-800 font-display tracking-tight leading-none">
      ...
      <div className="p-3">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-1.5">
            <span
              className={`px-2 py-0.5 rounded-md text-[11px] font-medium ${typeConfig.bg} ${typeConfig.text} flex items-center gap-1`}
            >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
      ...
      <div className="relative bg-white rounded-xl w-full max-w-xl max-h-[85vh] overflow-hidden flex flex-col shadow-2xl animate-scale-in">
        {/* 顶部彩条 */}
        <div
          className={`h-1 bg-gradient-to-r ${selectedTypeConfig?.gradient || "from-amber-400 to-orange-400"}`}
        ></div>
        ...
        <div className="px-4 py-3 border-b border-stone-100 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div
              className={`w-8 h-8 rounded-lg bg-gradient-to-br ${selectedTypeConfig?.gradient || "from-amber-400 to-orange-400"} flex items-center justify-center shadow-sm`}
            >
              {note ? (
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
          ...
          {!note && (
            <div>
              <label className="text-xs font-medium text-stone-600 block mb-2">笔记类型</label>
              <div className="grid grid-cols-3 gap-1.5">
                {NOTE_TYPE_OPTIONS.map((type) => {
                  ...
                  return (
                    ...
                    >
                      {TypeIcon && (
                        <TypeIcon className={`w-4 h-4 ${isSelected ? "" : "text-stone-400"}`} />
                      )}
          ...
          {/* Tags */}
          <div>
            <label className="text-xs font-medium text-stone-600 block mb-1.5">标签</label>
            <div className="flex items-center gap-2 mb-2">
...
export default function NotesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      if (tab === "my") {
        const [statsRes, notesRes] = await Promise.all([getMyNoteStats(), getMyNotes(params)]);
        setStats(statsRes);
  ...
  const handleSaveNote = async (data: CreateNoteRequest | UpdateNoteRequest, id?: number) => {
    try {
      if (id) {
  ...
  const handleLike = async (id: number) => {
    try {
      await likeNote(id);
      setNotes((prev) =>
        prev.map((n) => (n.id === id ? { ...n, is_liked: true, like_count: n.like_count + 1 } : n))
      );
    } catch (error) {
  ...
  const handleUnlike = async (id: number) => {
    try {
      ...
      setNotes((prev) =>
        prev.map((n) =>
          n.id === id ? { ...n, is_liked: false, like_count: Math.max(0, n.like_count - 1) } : n
        )
      );
    } catch (error) {
            ...
            {total > params.page_size! && (
              <div className="flex items-center justify-center gap-2 mt-6">
                ...
                <div className="flex items-center gap-1">
                  {Array.from(
                    { length: Math.min(5, Math.ceil(total / params.page_size!)) },
                    (_, i) => {
                      const pageNum = i + 1;
                      return (
                        <button
