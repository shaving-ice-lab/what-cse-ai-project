    ...
    <ul className="space-y-1.5">
      {items.map((item, index) => (
        <li key={index} className="flex items-start gap-2 text-sm">
          <IconComponent className={cn("h-3.5 w-3.5 mt-0.5 flex-shrink-0", iconColor)} />
    ...
    <ol className="space-y-1.5">
      {items.map((item, index) => (
        <li key={index} className="flex items-start gap-2 text-sm">
          ...
          <span className="text-muted-foreground pt-0.5">{String(item)}</span>
        </li>
      ))}
    </ol>
  );
  ...
  if (collapsible && onToggle) {
    return (
      <button
        type="button"
        onClick={onToggle}
        className="w-full text-left py-2 hover:bg-muted/30 rounded-md px-2 -mx-2 transition-colors"
      >
        {content}
      </button>
    );
...
const InfoBlock = ({
  ...
  const variantStyles = {
    ...
    warning: "bg-amber-50/50 dark:bg-amber-900/10 border-amber-200/50 dark:border-amber-800/30",
    success: "bg-emerald-50/50 dark:bg-emerald-900/10 border-emerald-200/50 dark:border-emerald-800/30",
    info: "bg-blue-50/50 dark:bg-blue-900/10 border-blue-200/50 dark:border-blue-800/30",
  };
  return (
    <div className={cn("rounded-md p-2.5 border text-sm", variantStyles[variant])}>
      {title && <div className="font-medium text-xs text-muted-foreground mb-1">{title}</div>}
...
const ConceptItem = ({ concept, index }: { concept: UnknownRecord; index: number }) => (
  <div className="py-3 space-y-2">
    <div className="flex items-center gap-2">
      <Badge variant="outline" className="text-xs bg-violet-50 dark:bg-violet-900/20 border-violet-200 dark:border-violet-800">
        概念 {index + 1}
      </Badge>
    ...
    {isNonEmptyArray(concept.application_scenarios) && (
      <StyledList items={concept.application_scenarios} icon={MapPin} iconColor="text-emerald-500" />
    )}
    ...
    {isNonEmptyArray(concept.common_pairs) && (
      <div className="flex flex-wrap gap-1.5">
        {(concept.common_pairs as string[]).map((pair, i) => (
          <Badge key={i} variant="secondary" className="text-xs">{pair}</Badge>
        ))}
      </div>
    )}
...
const MethodStepItem = ({ step, index }: { step: UnknownRecord; index: number }) => (
  <div className="py-3 space-y-2">
    ...
    {isNonEmptyArray(step.verification_checklist) && (
      <StyledList items={step.verification_checklist} icon={CheckCircle2} iconColor="text-emerald-500" />
    )}
  </div>
);
...
const MistakeItem = ({ mistake, index }: { mistake: UnknownRecord; index: number }) => (
  <div className="py-3 space-y-2">
    <div className="flex items-center gap-2">
      ...
      <span className="font-medium text-sm">{String(mistake.mistake || `易错点 ${index + 1}`)}</span>
      {isNonEmptyString(mistake.frequency) && (
        <Badge variant="outline" className="text-xs border-red-200 text-red-600">{mistake.frequency}</Badge>
      )}
    </div>
...
const StrategyItem = ({ strategy, index }: { strategy: UnknownRecord; index: number }) => (
  <div className="py-2 space-y-1.5">
    <div className="flex items-center gap-2">
      ...
      <span className="font-medium text-sm">{String(strategy.strategy || `策略 ${index + 1}`)}</span>
    </div>
  ...
  <div className="py-3 space-y-2">
    <div className="flex items-center justify-between flex-wrap gap-2">
      ...
      {isNonEmptyString(problem.knowledge_point) && (
        <Badge variant="secondary" className="text-xs">{problem.knowledge_point}</Badge>
      )}
    </div>
    <div className="bg-muted/30 rounded-md p-2.5 text-sm">
    ...
    {isNonEmptyArray(problem.options) && (
      <div className="space-y-1 pl-2">
        {(problem.options as string[]).map((option, i) => (
          <div key={i} className="text-sm text-muted-foreground">{option}</div>
        ))}
      </div>
    )}
    ...
    {isNonEmptyString(problem.advanced_technique) && (
      <InfoBlock title="高阶技巧" content={problem.advanced_technique as string} variant="success" />
    )}
  </div>
);

// Section item for lesson sections
const LessonSectionItem = ({ section, index, mermaidId }: { section: UnknownRecord; index: number; mermaidId: string }) => (
  <div className="py-3 space-y-3">
          ...
          {(section.real_exam_questions as UnknownRecord[]).map((item, idx) => (
            <div key={idx} className="py-2 space-y-1.5">
              ...
              {isNonEmptyString(item.quick_analysis) && (
                <InfoBlock title="快速分析" content={item.quick_analysis as string} variant="info" />
              )}
            </div>
          ))}
        </div>
    ...
    {isNonEmptyArray(section.key_takeaways) && (
      <ContentBlock title="要点总结" icon={Lightbulb} iconColor="text-amber-500">
        <StyledList items={section.key_takeaways} icon={CheckCircle2} iconColor="text-emerald-500" />
      </ContentBlock>
    )}
...
function ModuleStatusReport({
  ...
  return (
    <div className={cn(
      ...
      hasIssues ? "border-amber-300 bg-amber-50/50 dark:bg-amber-950/20" : "border-muted bg-muted/30"
    )}>
      <button
            ...
            {Object.entries(moduleStatus).map(([key, info]) => (
              ...
              >
                ...
                {info.required && !info.present && (
                  <Badge variant="destructive" className="text-[10px] h-4 px-1">必需</Badge>
                )}
              </div>
            ))}
          </div>
...
export function LessonContentPreview({ content }: { content: any }) {
  ...
  const moduleStatus = {
    ...
    introduction: { name: "课程导入", present: isNonEmptyString(lesson?.introduction), required: true },
    learning_goals: { name: "学习目标", present: isNonEmptyArray(lesson?.learning_goals), required: true },
    core_concepts: { name: "核心概念", present: isNonEmptyArray(lesson?.core_concepts), required: true },
    method_steps: { name: "方法步骤", present: isNonEmptyArray(lesson?.method_steps), required: true },
    formulas: { name: "记忆口诀", present: isNonEmptyArray(lesson?.formulas), required: true },
    memory_tips: { name: "记忆技巧", present: isNonEmptyArray(lesson?.memory_tips), required: true },
    common_mistakes: { name: "易错陷阱", present: isNonEmptyArray(lesson?.common_mistakes), required: true },
    exam_strategies: { name: "应试策略", present: isNonEmptyArray(lesson?.exam_strategies), required: true },
    vocabulary_accumulation: { name: "高频词汇", present: !!lesson?.vocabulary_accumulation, required: true },
    extension_knowledge: { name: "拓展知识", present: isNonEmptyString(lesson?.extension_knowledge), required: true },
    summary_points: { name: "课程总结", present: isNonEmptyArray(lesson?.summary_points), required: true },
    mind_map_mermaid: { name: "思维导图", present: isNonEmptyString(lesson?.mind_map_mermaid), required: true },
    ...
    practice_problems: { name: "练习题目", present: isNonEmptyArray(practiceProblems), required: true },
    homework: { name: "课后作业", present: !!homework && (isNonEmptyArray(homework.required) || isNonEmptyArray(homework.optional)), required: true },
  };
  ...
  return (
    <div className="space-y-1">
      ...
      <section>
        ...
        {meta.chapterTitle && (
          <h2 className="text-lg font-bold mb-3">{meta.chapterTitle}</h2>
        )}
        <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm">
          {isNonEmptyString(meta.subject) && (
            <KeyValue label="科目" value={meta.subject} />
          )}
          ...
          {meta.wordCountTarget && (
            <KeyValue label="目标字数" value={meta.wordCountTarget} />
          )}
          {wordCount?.total && (
            <KeyValue label="实际字数" value={wordCount.total} />
          )}
        </div>
      </section>
        ...
        <>
          <section>
            ...
            {isNonEmptyArray(exam.exam_forms) && (
              <div className="flex flex-wrap gap-1.5 mt-3">
                ...
                {(exam.exam_forms as string[]).map((form, i) => (
                  <Badge key={i} variant="outline" className="text-xs">{form}</Badge>
                ))}
              </div>
            )}
            ...
            {isNonEmptyString(exam.recent_trends) && (
              <div className="mt-2">
                <InfoBlock title="近年趋势" content={exam.recent_trends as string} variant="info" />
              </div>
            )}
          </section>
          <Separator className="my-4" />
        ...
        <>
          <section>
            ...
            {isNonEmptyArray(lesson.prerequisites) && (
              <div className="mb-4">
                <ContentBlock title="前置知识" icon={BookMarked} iconColor="text-violet-500">
                  <StyledList items={lesson.prerequisites} icon={CheckCircle2} iconColor="text-violet-500" />
                </ContentBlock>
              </div>
            )}
            ...
            {isNonEmptyArray(lesson.common_mistakes) && (
              <div className="mb-4">
                <ContentBlock title="易错点" icon={AlertTriangle} iconColor="text-red-500">
                  <div className="divide-y">
                  ...
                  <div className="space-y-3">
                    {isNonEmptyArray(lesson.vocabulary_accumulation.must_know) && (
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {(lesson.vocabulary_accumulation.must_know as string[]).map((word, i) => (
                    ...
                    {isNonEmptyArray(lesson.vocabulary_accumulation.should_know) && (
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {(lesson.vocabulary_accumulation.should_know as string[]).map((word, i) => (
                            <Badge key={i} variant="outline" className="text-xs">{word}</Badge>
                          ))}
                        </div>
                      </div>
                    ...
                    {isNonEmptyArray(lesson.vocabulary_accumulation.nice_to_know) && (
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {(lesson.vocabulary_accumulation.nice_to_know as string[]).map((word, i) => (
                            <Badge key={i} variant="outline" className="text-xs">{word}</Badge>
                          ))}
                        </div>
                      </div>
            ...
            {isNonEmptyString(lesson.extension_knowledge) && (
              <div className="mb-4">
                <InfoBlock title="拓展知识" content={lesson.extension_knowledge as string} variant="info" />
              </div>
            )}
                ...
                <ContentBlock title="快速笔记" icon={FileText} iconColor="text-amber-500">
                  <div className="space-y-3">
                      ...
                      <div className="space-y-2">
                        ...
                        {(lesson.quick_notes.formulas as UnknownRecord[]).map((item, index) => (
                          <div key={index} className="py-1.5">
                            ...
                            <p className="text-sm font-mono bg-muted/30 rounded px-2 py-1 mt-1">{String(item.content || "")}</p>
                    ...
                    {isNonEmptyArray(lesson.quick_notes.key_points) && (
                      <div>
                        <h5 className="text-xs font-medium text-muted-foreground mb-1.5">要点速记</h5>
                        <StyledList items={lesson.quick_notes.key_points} icon={Star} iconColor="text-amber-500" />
                      </div>
                    )}
                    {isNonEmptyArray(lesson.quick_notes.common_mistakes) && (
                      <div className="space-y-2">
                        <h5 className="text-xs font-medium text-muted-foreground">易错速记</h5>
                        {(lesson.quick_notes.common_mistakes as UnknownRecord[]).map((item, index) => (
                          <div key={index} className="py-1.5">
                            <div className="flex items-center gap-1.5">
                              <AlertTriangle className="h-3.5 w-3.5 text-red-500" />
                              <span className="font-medium text-sm">{String(item.mistake || "")}</span>
                            </div>
                            <Paragraph value={item.correction} className="text-emerald-600 dark:text-emerald-400 mt-1" />
                          </div>
                        ))}
                      </div>
                    )}
                    {isNonEmptyArray(lesson.quick_notes.exam_tips) && (
                      <div>
                        <h5 className="text-xs font-medium text-muted-foreground mb-1.5">考试技巧</h5>
                        <StyledList items={lesson.quick_notes.exam_tips} icon={Zap} iconColor="text-violet-500" />
                      </div>
                    )}
                  </div>
                </ContentBlock>
          ...
          <div className="divide-y">
            {(modules as UnknownRecord[]).map((module, index) => (
              <div key={index} className="py-3">
                ...
                <Paragraph value={module.content} />
              </div>
            ))}
          </div>
        </section>
