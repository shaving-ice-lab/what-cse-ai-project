...
export function TaskDetailDialog({ open, onOpenChange, task }: TaskDetailDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="max-w-[1100px] h-[90vh] p-0 flex flex-col overflow-hidden gap-0"
        hideCloseButton
      >
          ...
          <div className="relative px-6 py-4">
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex items-start gap-4 min-w-0 flex-1">
                <div
                  className={`p-2.5 rounded-xl bg-gradient-to-br ${config.gradient} text-white shadow-lg flex-shrink-0`}
                >
                ...
                <div className="min-w-0 flex-1 pt-0.5">
                  ...
                  <div className="flex items-center gap-3 mt-2 text-sm text-muted-foreground">
                    <span>
                      任务ID: <code className="font-mono text-foreground">{task.id}</code>
                    </span>
                    <span>·</span>
                    {task.startTime && (
        ...
        <div className="flex-shrink-0 px-6 py-2.5 bg-muted/40 border-y flex items-center gap-4 text-sm">
          ...
          {task.parseResult?.data?.final_url &&
            task.parseResult.data.final_url !== task.fenbiUrl && (
            ...
            <div className="flex items-center gap-2 ml-auto text-muted-foreground">
              ...
              >
                ...
                <Badge variant="outline" className="text-[10px] h-4 ml-1">
                  {task.parseResult.data.list_page_source === "url_parse"
                    ? "URL"
                    : task.parseResult.data.list_page_source === "html_extract"
                      ? "HTML"
                      : task.parseResult.data.list_page_source === "llm"
                        ? "LLM"
                        : ""}
                </Badge>
                <ExternalLink className="h-3 w-3" />
                          ...
                          ${
                            isCompleted
                              ...
                              : isFailed
                                ...
                                : isRunning
                                  ? "bg-blue-50/80 dark:bg-blue-950/40 border-blue-300 dark:border-blue-700 ring-2 ring-blue-400/50"
            ...
            {stats && stats.totalSteps > 0 && (
              <div className="flex-shrink-0 px-4 py-3 border-t bg-muted/30 space-y-1 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-semibold ${stats.failedCount ? "text-amber-600" : "text-emerald-600"}`}
                  >
                    {stats.successCount}/{stats.totalSteps}
                  </span>
                </div>
          ...
          <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
            {hasParseResult ? (
              <Tabs defaultValue="ai" className="flex-1 flex flex-col min-h-0 overflow-hidden">
                ...
                <div className="flex-shrink-0 h-[49px] px-4 border-b bg-muted/20 flex items-center">
                  <TabsList className="h-9 p-1 bg-muted/60 rounded-lg gap-1">
                    <TabsTrigger
                      value="ai"
                      className="h-7 px-3 text-sm rounded-md data-[state=active]:bg-white dark:data-[state=active]:bg-slate-800 data-[state=active]:shadow-sm"
                    >
                ...
                <div className="flex-1 min-h-0 relative">
                                ...
                                <div className="p-5 bg-emerald-50/50 dark:bg-emerald-950/30 rounded-xl border border-emerald-200/60 dark:border-emerald-800/60">
                                  ...
                                  <div className="space-y-2 max-h-48 overflow-auto">
                                    {positions.slice(0, 5).map((pos, idx) => (
                                      <div
                                        key={idx}
                                        className="p-3 bg-white dark:bg-slate-800 rounded-lg border flex items-center justify-between"
                                      >
                                        <div className="flex items-center gap-2 min-w-0">
                                          ...
                                          {pos.department_name && (
                                            <Badge
                                              variant="outline"
                                              className="text-[10px] h-5 ml-1 truncate max-w-[120px]"
                                            >
                                              {pos.department_name}
                                            </Badge>
                                          )}
                          ...
                          <div>
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <Badge variant="outline">
                                {task.parseResult.data.page_content.length.toLocaleString()} 字符
                              </Badge>
                            </div>
                  ...
                  <TabsContent
                    value="positions"
                    className="absolute inset-0 m-0 flex flex-col overflow-hidden data-[state=inactive]:hidden"
                  >
                    <ScrollArea className="h-full">
                      <div className="p-5 space-y-4">
                        {positions.length > 0 ? (
                            ...
                            <div className="grid grid-cols-3 gap-3">
                              ...
                              <div className="p-4 bg-violet-50 dark:bg-violet-950/40 rounded-xl border border-violet-200 dark:border-violet-800 text-center">
                                <div className="text-3xl font-bold text-violet-600 dark:text-violet-400">
                                  {
                                    new Set(positions.map((p) => p.department_name).filter(Boolean))
                                      .size
                                  }
                                </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground p-8">
                <div
                  className={`p-8 rounded-full ${config.light} border-2 border-dashed ${config.border}`}
                >
        ...
        <div className="flex-shrink-0 px-6 py-3 bg-muted/30 flex items-center justify-between">
          <div className="text-xs text-muted-foreground">
            {llm?.summary && (
              <span className="truncate max-w-[350px] inline-block">
                {llm.summary.slice(0, 80)}...
              </span>
            )}
          </div>
          <div className="flex items-center gap-2">
