...
export const materialApi = {
  ...
  // 获取所有分类
  getCategories: () => request.get<MaterialCategory[]>("/materials/categories"),

  // 获取分类树
  getCategoryTree: () => request.get<MaterialCategory[]>("/materials/categories/tree"),
  ...
  getCategoriesBySubject: (subject: string) =>
    request.get<MaterialCategory[]>(`/materials/categories/subject/${encodeURIComponent(subject)}`),
  ...
  // 删除素材
  deleteMaterial: (id: number) => request.delete<void>(`/admin/materials/${id}`),
  ...
  // 获取预置数据信息
  getPresetDataInfo: () => request.get<PresetDataInfo>("/admin/materials/preset/info"),
  ...
  // 删除分类
  deleteCategory: (id: number) => request.delete<void>(`/admin/materials/categories/${id}`),
};
