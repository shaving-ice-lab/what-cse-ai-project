...
export default function PositionSearchPage() {
  ...
  const [hotKeywords] = useState(["国考2024", "省考", "税务", "海关", "统计", "事业编"]);
    ...
    <View className="search-page">
      <View className="search-header">
        <View className="search-box">
          ...
          />
          {keyword && (
            <Text className="clear-btn" onClick={() => setKeyword("")}>
              ✕
            </Text>
          )}
        </View>
        ...
        {searchHistory.length > 0 && (
          <View className="search-section">
            <View className="section-header">
              ...
              <Text className="clear-all" onClick={handleClearHistory}>
                清空
              </Text>
            </View>
            <View className="keyword-list">
              {searchHistory.map((item, index) => (
                <Text key={index} className="keyword-tag" onClick={() => handleSearch(item)}>
                  {item}
                </Text>
              ))}
            </View>
        ...
        <View className="search-section">
          ...
          <View className="keyword-list">
            {hotKeywords.map((item, index) => (
              <Text key={index} className="keyword-tag hot" onClick={() => handleSearch(item)}>
                {item}
              </Text>
            ))}
          </View>
