...
- [x] 表单组件封装
  ...
  - [x] 创建 `Button` - 按钮组件
  - [ ] 创建 `FormDatePicker` - 日期选择器
  - [ ] 创建 `RegionSelect` - 省市区三级联动
  - [ ] 创建 `MajorSelect` - 专业选择器
...
- [x] 业务组件
  ...
  - [ ] 创建 `ConditionCompareTable` - 条件对比表
  - [ ] 创建 `PositionTimeline` - 岗位生命周期时间线
